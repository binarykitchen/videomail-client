...
class Form extends Despot {
  ...
  public build() {
    ...
    if (this.options.enableAutoValidation) {
      ...
      for (let i = 0, len = inputElements.length; i < len; i++) {
        ...
        if (type === "radio" || type === "select") {
          inputElement?.addEventListener("change", this.container.validate.bind(this));
        } else {
          inputElement?.addEventListener("input", this.container.validate.bind(this));
        }
      }
    }

    this.on(Events.PREVIEW, (videomailKey?: string) => {
      /*
      ...
      if (!videomailKey && !this.keyInput.value) {
        ...
        this.keyInput.value = videomailKey;
    ...
    this.on(Events.ERROR, (err: VideomailError) => {
      /*
      ...
      if (this.options.adjustFormOnBrowserError) {
        this.hideAll();
      } else if (err.isBrowserProblem()) {
        this.hideSubmitButton();
      }
    });

    this.on(Events.BUILT, () => {
  ...
  private removeAllInputListeners() {
    ...
    for (const inputElement of inputElements) {
      ...
      if (type === "radio" || type === "select") {
        inputElement.removeEventListener("change", this.container.validate.bind(this));
      } else {
        inputElement.removeEventListener("input", this.container.validate.bind(this));
      }
    }
  }
  ...
  public doTheSubmit(e?) {
    if (e) {
      this.options.logger.debug(`Form: doTheSubmit(${stringify(e)})`);
      e.preventDefault();
    } else {
