...
class Visuals extends Despot {
  ...
  private buildChildren(playerOnly = false, replayParentElement?: HTMLElement | null) {
    if (!replayParentElement) {
      throw new Error("Unable to build replay without a parent element");
    }

    this.options.logger.debug(
      `Visuals: buildChildren (playerOnly = ${playerOnly}, replayParentElement="${pretty(replayParentElement)}"})`,
    );
    ...
    this.replay.build(replayParentElement);
  }
  ...
  private initEvents(playerOnly = false) {
    if (!playerOnly) {
      ...
      this.on(Events.USER_MEDIA_READY, () => {
        this.built = true;
        this.endWaiting();
  ...
  public build(playerOnly = false, replayParentElement?: HTMLElement | null) {
    this.options.logger.debug(
      `Visuals: build (playerOnly = ${playerOnly}${replayParentElement ? `, replayParentElement="${pretty(replayParentElement)}"` : ""})`,
    );

    if (replayParentElement) {
      this.visualsElement = replayParentElement.querySelector<HTMLElement>(
    ...
    if (!this.visualsElement) {
      if (playerOnly && replayParentElement) {
        this.visualsElement = replayParentElement;
      } else {
  ...
  public recordAgain() {
    this.back(false, () => {
      if (this.options.loadUserMediaOnRecord) {
        this.once(Events.SERVER_READY, () => {
          this.recorder.record();
        });
      } else {
        this.once(Events.USER_MEDIA_READY, () => {
          this.recorder.record();
        });
      }
    });
  ...
  public unload(e) {
    try {
      ...
      this.options.logger.debug(`Visuals: unload(${e ? stringify(e) : ""})`);

      this.removeAllListeners();
      this.recorder.unload(e);
