...
class Notifier extends Despot {
  ...
  private messageElement?: HTMLElement | null | undefined;
  ...
  private initEvents() {
    ...
    this.on(Events.CONNECTING, () => {
      this.onConnecting();
    })
      .on(Events.LOADING_USER_MEDIA, () => {
        this.onLoadingUserMedia();
      })
      .on(Events.USER_MEDIA_READY, () => {
      ...
      .on(Events.STOPPING, (limitReached) => {
        this.onStopping(limitReached);
      })
      .on(Events.PROGRESS, (frameProgress, sampleProgress) => {
        this.onProgress(frameProgress, sampleProgress);
      })
      .on(Events.BEGIN_VIDEO_ENCODING, () => {
  ...
  public error(err: VideomailError) {
    ...
    if (!message) {
      this.options.logger.debug("Weird empty error message generated for error", err);
    }
  ...
  private getMessageElement() {
    ...
    this.messageElement = document.getElementById(NOTIFIER_MESSAGE_ID);
