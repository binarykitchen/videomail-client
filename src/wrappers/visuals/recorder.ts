...
import calculateWidth from "../../util/dimensions/calculateWidth";
import calculateHeight from "../../util/dimensions/calculateHeight";
...
import getRatio from "../../util/dimensions/getRatio";
...
class Recorder extends Despot {
  ...
  private onUserMediaReady(params?: UserMediaParams) {
    try {
      this.options.logger.debug("Recorder: onUserMediaReady()", stringify(params));
      ...
      this.emit(Events.USER_MEDIA_READY, {
  ...
  private updateFrameProgress(frame: number) {
    this.confirmedFrameNumber = frame;
  ...
  private updateSampleProgress(sample: number) {
    this.confirmedSampleNumber = sample;
  ...
  private preview(args: PreviewArgs) {
    ...
    this.emit(Events.PREVIEW, { key: this.key, width, height, hasAudio });
    ...
    this.options.logger.debug(
      ...
      filesize(this.bytesSum ?? -1, { round: 2 }),
      humanizeDuration(this.waitingTime),
    );
  }
  ...
  private initSocket(cb?: () => void) {
    if (!this.connected) {
      ...
      this.options.logger.debug(
        "Recorder: initializing web socket to %s",
      ...
      if (this.stream) {
        ...
        this.stream.on("error", (err) => {
          this.options.logger.debug(
            `${PIPE_SYMBOL}Stream *error* event emitted: ${stringify(err)}`,
          );
        });
  ...
  private showUserMedia() {
    ...
    return (this.isNotifying() ?? !this.isHidden()) || this.blocking;
  }
  ...
  private userMediaErrorCallback(err) {
    ...
    this.options.logger.debug(
      `Recorder: userMediaErrorCallback(), name: ${err.name}, message: ${err.message} and Webcam characteristics: ${stringify(this.userMedia?.getCharacteristics())}`,
    );

    const errorListeners = this.listeners(Events.ERROR);

    if (errorListeners.length) {
      if (err.name !== VideomailError.MEDIA_DEVICE_NOT_SUPPORTED) {
        const videomailError = createError({ err, options: this.options });
        this.emit(Events.ERROR, { err: videomailError });
      } else {
        // do not emit but retry since MEDIA_DEVICE_NOT_SUPPORTED can be a race condition
        this.options.logger.debug("Recorder: ignore user media error", err);
      }
       ...
       */
      this.options.logger.debug(
        "Recorder: already unloaded. Not going to throw error",
        err,
      );
    } else {
      this.options.logger.debug(
        "Recorder: no error listeners attached but throwing error",
        err,
      );
  ...
  private getUserMediaCallback(localStream: MediaStream, params?: UserMediaParams) {
    if (!this.userMedia) {
      throw new Error("No user media is defined");
    }

    this.options.logger.debug("Recorder: getUserMediaCallback()", stringify(params));
  ...
  private loadGenuineUserMedia(params?: UserMediaParams) {
    this.options.logger.debug(`Recorder: loadGenuineUserMedia(${params})`);

    this.emit(Events.ASKING_WEBCAM_PERMISSION);
    ...
    this.options.logger.debug(
      `Recorder: navigator.mediaDevices.getUserMedia() ${stringify(constraints)}`,
    );

    this.options.logger.debug(
      `Recorder: navigator.mediaDevices.getSupportedConstraints() ${stringify(navigator.mediaDevices.getSupportedConstraints())}`,
    );
  ...
  private loadUserMedia(params?: UserMediaParams) {
    ...
    this.options.logger.debug(`Recorder: loadUserMedia(${stringify(params)})`);

    this.emit(Events.LOADING_USER_MEDIA);

    try {
      this.userMediaTimeout = window.setTimeout(() => {
        if (!this.isReady()) {
          const message = getBrowser(this.options).getNoAccessIssue();
      ...
      if (errorListeners.length) {
        ...
        this.options.logger.debug(
          "Recorder: no error listeners attached but throwing exception",
          exc,
        );
  ...
  private executeCommand(command: Command) {
    ...
    try {
      if (command.args) {
        this.options.logger.debug(
          `Server commanded: ${command.command} with ${stringify(command.args)}`,
        );
      } else {
  ...
  private writeCommand(command: string, args: any, cb?: () => void) {
    if (!this.connected) {
      ...
      if (args) {
        this.options.logger.debug(`$ ${command} with ${stringify(args)}`);
      } else {
      ...
      this.writeStream(Buffer.from(stringify(commandObj)));
  ...
  public stop(params?: StopParams) {
    ...
    this.options.logger.debug(`stop(${stringify(params)})`);
    ...
    setTimeout(() => {
      ...
      this.writeCommand("stop", this.recordingStats, () => {
        this.emit(Events.STOPPED, { recordingStats: this.recordingStats });
      });
  ...
  public pause(params?: PauseParams) {
    if (params) {
      this.options.logger.debug(
        `pause() at frame ${this.framesCount} with ${stringify(params)}`,
      );
    } else {
  ...
  private onFlushed(opts: WriteStreamParams) {
    ...
    if (frameNumber === 1) {
      this.emit(Events.FIRST_FRAME_SENT);
    }
  }
    ...
    try {
      ...
      if (!this.isPaused() && this.stream && this.ctx) {
        if (this.framesCount === 0) {
          this.emit(Events.SENDING_FIRST_FRAME);
        }
        ...
        if (!recordingBufferLength) {
          ...
          const frameControlBuffer = Buffer.from(
            stringify({ frameNumber: this.framesCount }),
          );
  ...
  public record() {
    ...
    if (!this.connected) {
      ...
      this.initSocket(() => {
        this.once(Events.USER_MEDIA_READY, this.record.bind(this));
      });
    ...
    this.emit(Events.RECORDING, { framesCount: this.framesCount });
  ...
  public getRecorderHeight(responsive: boolean, useBoundingClientRect?: boolean) {
    if (this.recorderElement && useBoundingClientRect) {
      ...
      this.calculateHeight(responsive);
      return;
    }
