...
class Replay extends Despot {
  ...
  public show(
    recorderWidth: number | undefined,
    recorderHeight: number | undefined,
    hasAudio: boolean,
    playerOnly = false,
  ) {
    if (!this.replayElement) {
    ...
    if (this.videomail) {
      this.correctDimensions(
        ...
        recorderWidth ?? this.replayElement.videoWidth,
        recorderHeight ?? this.replayElement.videoHeight,
      );
    }
  ...
  public build(replayParentElement: HTMLElement) {
    ...
    if (!this.built) {
      if (!this.isStandalone()) {
        this.on(Events.PREVIEW, ({ width, height, hasAudio }) => {
          this.show(width, height, hasAudio);
        });
      }
